...
//int Fun4All_G4_EicDirc(const int nEvents = 1000, const string &outputFile = "G4DIRCTree.root", const string &outputPath = ".", const char *DSToutfile = NULL)
{
  ...
  */
  ...
  eicdirc->set_double_param("NBars", 5);
  eicdirc->set_double_param("Radius", 50.17 * cm);
  eicdirc->set_double_param("Prizm_width", 17.56 * cm);  
  ...
  eicdirc->set_double_param("BarS_length", 35.0 * cm);
  eicdirc->set_double_param("Mirror_height", 2.0 * cm);
  eicdirc->set_double_param("z_shift", 0 * cm);
  eicdirc->set_int_param("Geom_type", 1); // 0-whole DIRC, 1-one bar box
  eicdirc->set_int_param("Lens_id", 3); // 3- 3-layer spherical lens
  eicdirc->set_int_param("MCP_rows", 3);
  eicdirc->set_int_param("MCP_columns", 4);
  eicdirc->set_int_param("NBoxes", 16); // number of bar boxes
  eicdirc->set_int_param("Bar_pieces", 3); // pieces glued in one bar
